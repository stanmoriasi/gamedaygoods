...
const Checkout = () => {
 ...
 const [products, setProducts] =useState<cartItem[]>([]);
  ...
  useEffect(()=> {
    ...
    const cartTotal = cart.reduce((sum: number, b: cartItem) => sum + (b.price * b.quantity), 0);
    setProducts(cart);
    setTotal(cartTotal);
  }, [])
    ...
    const handleFormSubmit = async (event: FormEvent) => {
      ...
      const mappedProducts = products.map((prod) => ({productId: prod._id, quantity: prod.quantity}));
